  ...
  const runFullAnalysisPipeline = useCallback(async () => {
    ...
    try {
      ...
      if (CAD_MODEL_FORMATS.includes(ext)) {
        ...
        if (viewPayload) {
          // Select 4 best views for AI analysis: 2 orthographic + 2 perspective
          const selectedViews: GeneratedView[] = [];

          // Priority orthographic views: front and top (most informative)
          const frontView = viewPayload.orthographic.find(v => v.name === 'front');
          const topView = viewPayload.orthographic.find(v => v.name === 'top');

          if (frontView) selectedViews.push(frontView);
          if (topView) selectedViews.push(topView);

          // If we don't have front/top, use the first 2 available orthographic views
          if (selectedViews.length < 2) {
            const remainingOrtho = viewPayload.orthographic.filter(
              v => !selectedViews.includes(v)
            );
            selectedViews.push(...remainingOrtho.slice(0, 2 - selectedViews.length));
          }

          // Add 2 perspective views for 3D context
          selectedViews.push(...viewPayload.perspective.slice(0, 2));

          combinedViews = selectedViews;
