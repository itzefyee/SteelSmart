...
const renderHighlights = (placement: HighlightPlacement) => {
            ...
            <div key={highlight.label} className={cardClasses}>
              <p
                className={`text-[0.65rem] font-semibold uppercase tracking-[0.25em] ${
                  isLight ? 'text-slate-500' : 'text-blue-200/80'
                }`}
              >
                {highlight.label}
              </p>
  ...
  const renderTitleBlock = () => (
    <div className="space-y-2">
      ...
      <p
        className={`text-base sm:text-lg ${descriptionConstraints} max-w-3xl ${
  ...
  return (
    <section
      className={`relative rounded-[32px] px-3 py-2 sm:px-4 sm:py-2 ${
        isLight ? 'text-slate-900' : 'text-white'
      }`}
    >
